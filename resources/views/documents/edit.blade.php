...
<div class="py-6">
    <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div class="md:flex md:items-center md:justify-between mb-6">
            ...
            <a href="{{ route('documents.show', $document) }}" class="text-sm text-gray-600 hover:text-gray-900">← Back to Document</a>
        </div>
            ...
            <form action="{{ route('documents.update', $document) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="excerpt" id="excerpt" rows="3"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('excerpt') border-red-500 @enderror">{{ old('excerpt', $document->excerpt) }}</textarea>
                    @error('excerpt')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="20"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('content') border-red-500 @enderror">{{ old('content', $document->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="change_summary" class="block text-sm font-medium text-gray-700">Change Summary (Optional)</label>
                    <input type="text" name="change_summary" id="change_summary" value="{{ old('change_summary') }}"
                        placeholder="Describe what you changed..."
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    <p class="mt-1 text-sm text-gray-500">This will be saved in the version history</p>
                </div>
